...
router.get('/oee-events', (req, res) => {
  ...
  const rows = db.prepare(`
    ...
    WHERE me.started_at >= datetime('now', ?)
...
router.get('/all', (req, res) => {
  const bundle = {
    ...
    apps: db.prepare('SELECT id, name, description, status, created_at, updated_at FROM apps').all(),
    ...
    inventory: db.prepare(`
      ...
      FROM items i LEFT JOIN stock_levels sl ON sl.item_id = i.id WHERE i.is_active=1 GROUP BY i.id
    `).all(),
    vendors: db.prepare('SELECT * FROM vendors WHERE is_active = 1').all(),
    purchase_orders: db.prepare(`
      SELECT po.*, v.name as vendor_name FROM purchase_orders po JOIN vendors v ON v.id = po.vendor_id ORDER BY po.order_date DESC
    `).all(),
    ncrs: db.prepare('SELECT * FROM ncrs ORDER BY created_at DESC').all(),
  };
